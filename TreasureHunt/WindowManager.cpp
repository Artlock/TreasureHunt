...
WindowManager::WindowManager(sf::RenderWindow* Window, Player* Player): _Window(Window),_Player(Player)
{
	...
	_FirstPositionWindow = new sf::Vector2i(_Window->getPosition().x + _Player->getPosX() - _Window->getSize().x / 2, _Window->getPosition().y + _Player->getPosY() - _Window->getSize().y / 2);
...
void WindowManager::UpdateWindow() 
{
	*_OldPositionWindow = _Window->getPosition();
	_Window->setPosition( sf::Vector2i( _FirstPositionWindow->x + _Player->getPosX() , _FirstPositionWindow->x + _Player->getPosY()) );
